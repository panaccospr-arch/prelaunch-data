...
export default function RegistrationForm() {
  // ... useState and handler functions remain the same ...
  const [formData, setFormData] = useState<FormData>({ fName: '', lName: '', gender: '', dateBirth: '', phoneNo: '', email_ID: '', pincode: '', country: 'Bharat (India)', state: '', district: '', city: '', policeStation: '', postOffice: '', locality: '', landmark: '', authorized: false, });
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => { const { name, value, type } = e.target; const isCheckbox = type === 'checkbox'; const checked = isCheckbox ? (e.target as HTMLInputElement).checked : false; setFormData((prev) => ({ ...prev, [name]: isCheckbox ? checked : value })); };
  const handleLocationClick = () => { alert('Google Maps location feature coming in Sprint 3!'); };
  const handleSubmit = (e: React.FormEvent) => { e.preventDefault(); console.log('Form Submitted!', formData); alert('Thank you for your contribution!'); };


  return (
    <div className="w-full max-w-xl p-6 sm:p-8 space-y-6 bg-white rounded-2xl shadow-xl border border-gray-200 text-left">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
             ...
             <button type="button" onClick={handleLocationClick} className="flex justify-center items-center py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">📍 Use my location</button>
          </div>
        </div>
        <div className="flex items-start space-x-3 pt-2">
          ...
          <label htmlFor="authorized" className="text-sm text-gray-600">I agree to the{' '}<a href="/terms-of-service" target="_blank" className="font-medium text-blue-600 hover:underline">Terms of Service</a>{' '}and{' '}<a href="/privacy-policy" target="_blank"className="font-medium text-blue-600 hover:underline">Privacy Policy</a>{' '}and consent to receive communications from Panacco.</label>
        </div>
        <button type="submit" className="w-full flex justify-center py-3 px-4 border border-transparent rounded-md shadow-sm text-lg font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-transform hover:scale-105">Submit Information</button>
      </form>
    </div>
  );
}